...
const resetBall = () => {
    ball.x = cvs.width / 2;
    ball.y = cvs.height / 2; //Buralar da kontrol edilecek.
...
const update = () => {
    if (!ball.stop) {
        ...
        ball.y += ball.velocityY; //Buray� dene
    }
    ...
    let comLvl = 0.1; //Buras� yanl�� olursa iften ��kat bunu. Fonksiyon i�erisine yaz.
    ...
    if (ball.x > cvs.width) {
        user.score++;
        resetBall();
    } else if(ball.x < 0){ //Buralar kontrol edilecek
        com.score++;
        resetBall();
    }
}
